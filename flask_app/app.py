...
def load_model_and_vectorizer(model_name, model_version, vectorizer_path):
    ...
    mlflow.set_tracking_uri("http://ec2-13-211-142-57.ap-southeast-2.compute.amazonaws.com:5000/")  # Replace with your MLflow tracking URI
    client = MlflowClient()
...
def predict_with_timestamps():
    ...
    try:
        ...
        # ✅ NEW: Create DataFrame with same feature names as vectorizer
        columns = vectorizer.get_feature_names_out()
        dense_df = pd.DataFrame(dense_comments, columns=columns)
...
def predict():
    ...
    try:
        ...
        # ✅ 3. Convert to DataFrame with correct column names (important!)
        import pandas as pd
        dense_comments = transformed_comments.toarray()
        columns = vectorizer.get_feature_names_out()
        dense_df = pd.DataFrame(dense_comments, columns=columns)

        # ✅ 4. Predict using MLflow model
        predictions = model.predict(dense_df).tolist()
